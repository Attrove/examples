...
describe.skipIf(
  ...
  it('should create durable connect session with activation artifacts', async () => {
    const result = await admin.users.createConnectSession(testUserId, {
      provider: 'gmail',
      includeInstall: true,
    });

    expect(result.session_id, 'Session ID should be defined').toBeDefined();
    expect(result.activation_url, 'Activation URL should be defined').toContain(
      '/integrations/connect?session=',
    );
    expect(result.cli?.command, 'CLI handoff should be defined').toContain(
      'npx @attrove/cli connect --session',
    );
    expect(result.expires_at, 'Expiration should be defined').toBeDefined();

    // Verify expiration is in the future (sessions are durable enough for async handoff)
